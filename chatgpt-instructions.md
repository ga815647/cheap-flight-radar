...
1. Repo is `ga815647/cheap-flight-radar` (github.com, branch `main` is source of truth). Read `AGENTS.md`, `PRODUCT_INTENT.md`, full `flight-radar.yaml` (SSOT wins), then `docs/daily-flight-radar-automation-prompt.md`. Resolve the requested date using the current `Asia/Taipei` calendar date.
...
Scheduling cadence and trigger time are owned by the ChatGPT automation, not by this Project Instruction.
