...
- Routine automation should run the Daily Radar about **once per day** for now via the ChatGPT-triggered GitHub run; no immediate-alert requirement. This is a scheduling preference, not a prohibition on an explicit same-day operator-requested reacquisition when the user wants refreshed evidence, diagnosis, or a provider-health comparison. (2026-09-18～09-25 local scheduled-run interlude superseded.)
...
- Scheduled notification decisions happen only after the triggered run reaches a terminal state and the final immutable run evidence in the repo data trees is read back. Meaningful new Deals and operational/provider/coverage failures notify; a healthy routine run with no meaningful change may stay silent. A UI notification toggle is only a delivery mechanism, not product policy.
