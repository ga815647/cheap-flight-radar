...
- Routine automation should run the Daily Radar about **once per day** for now on the owner's own machine via a scheduled local run (OpenChamber scheduled task); no immediate-alert requirement and no GitHub Actions in the production path. This is a scheduling preference, not a prohibition on an explicit same-day operator-requested reacquisition when the user wants refreshed evidence, diagnosis, or a provider-health comparison.
